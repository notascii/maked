...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        raise ValueError(
            "Wrong use of this file. Please use `python gen-makefile.py -n <name> -c <nb_commands> -s <sleep_time>"
        )
    name = None
    commands = None
    sleep = None
    ...
    if sleep is None or commands is None or name is None:
        raise ValueError(
            "Wrong use of this file. Please use `python gen-makefile.py -n <name> -b <nb_branches> -s <sleep_time>"
        )
    makefile_path = f"makefiles/{name}-c-{commands}-s-{sleep}"
